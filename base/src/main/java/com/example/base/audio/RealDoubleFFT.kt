...
class RealDoubleFFT(private val ndim: Int) {
    var norm_factor: Double
    private var wavetable: DoubleArray = DoubleArray(0)
    private val ch: DoubleArray // reusable work array


    fun ft(x: DoubleArray) {
        require(x.size == ndim) { "The length of data can not match that of the wavetable" }
            ...
            if (nbd <= l1) {
                ...
                while (j < ip) {
                    ...
                    while (i < ido) {
                        ...
                        while (k < l1) {
                            ch[i - 1 + (k + j * l1) * ido] =
                                (wtable!![idij - 1 + offset] * c1[i - 1 + (k + j * l1) * ido]
                                        + wtable[idij + offset] * c1[i + (k + j * l1) * ido])
                            ch[i + (k + j * l1) * ido] =
                                (wtable[idij - 1 + offset] * c1[i + (k + j * l1) * ido]
                ...
                while (j < ipph) {
                    ...
                    while (k < l1) {
                        ...
                        while (i < ido) {
                            ...
                            c1[i - 1 + (k + jc * l1) * ido] =
                                ch[i + (k + j * l1) * ido] - ch[i + (k + jc * l1) * ido]
                            c1[i + (k + j * l1) * ido] =
                                ch[i + (k + j * l1) * ido] + ch[i + (k + jc * l1) * ido]
                            c1[i + (k + jc * l1) * ido] =
                ...
                while (k < l1) {
                    ...
                    while (i < ido) {
                        ...
                        cc[i + (j2 + k * ip) * ido] =
                            ch[i + (k + j * l1) * ido] + ch[i + (k + jc * l1) * ido]
                        cc[ic + (j2 - 1 + k * ip) * ido] =
                            ch[i + (k + jc * l1) * ido] - ch[i + (k + j * l1) * ido]
                        i += 2
                    }
                    k++
                }
    ...
    fun rfftf1(n: Int, c: DoubleArray, wtable: DoubleArray?, offset: Int, ch: DoubleArray) {
        var i: Int
        var k1: Int
        var l1: Int
        var l2: Int
    ...
    fun rfftf(n: Int, r: DoubleArray, wtable: DoubleArray?, ch: DoubleArray) {
        if (n == 1) return
        rfftf1(n, r, wtable, 0, ch)
    }

    fun rffti1(n: Int, wtable: DoubleArray, offset: Int) {
    ...
    fun rffti(n: Int, wtable: DoubleArray) {
        if (n == 1) return
        rffti1(n, wtable, 0)
    }
    ...
    init {
        norm_factor = ndim.toDouble()
        wavetable = DoubleArray(0)
        if (wavetable.size != 2 * ndim + 15) {
